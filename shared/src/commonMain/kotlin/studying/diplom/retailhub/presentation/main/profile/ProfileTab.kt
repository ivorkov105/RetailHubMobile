		...
		LaunchedEffect(Unit) {
			...
			screenModel.navigationEvents.collectLatest { event ->
				when (event) {
					...
					ProfileNavigationEvent.NavigateToNotifications       -> {
						navigator.parent?.push(NotificationsScreen())
					}
				}
			}
		}
				...
				TopAppBar(
					...
					actions = {
						Box(modifier = Modifier.padding(end = 8.dp)) {
							IconButton(onClick = { screenModel.onEvent(ProfileEvent.OnNotificationsClick) }) {
								Icon(
